...
export interface McpSession { target?: McpTarget; expires_at: number; idle_seconds: number; session_id: string; target_id: string; state: string; integration_id: string; error: string | null }
...
export interface McpStatus {
  ...
  activity: { grants: { integration_id: string; remaining_seconds: number; targets: McpTarget[] }[]; sessions: McpSession[]; runs: McpRun[] };
}
...
export const mcpTargets = () => invoke<{ targets: McpTarget[]; ticket: string }>("mcp_targets");
export const mcpGrant = (id: string, targets: McpTarget[], seconds: number, ticket: string) => invoke<void>("mcp_grant", { id, targets: targets.map(({ vault_id, profile_id }) => ({ vault_id, profile_id })), seconds, ticket });
